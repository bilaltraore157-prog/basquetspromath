...
// --- MODULE PRÉDICTION MIS À JOUR (INDIVIDUEL + TOTAL) ---
function ajouterPredictionOverUnder(zone) {
    ...
    // Récupération des données du tableau pour les calculs
    const lignes = body.getElementsByTagName('tr');
    const celluleFoot = foot.getElementsByTagName('td');

    // --- CALCULS ÉQUIPE DOMICILE ---
    const domMin = parseFloat(lignes[0].getElementsByTagName('td')[1].innerText);
    const domMax = parseFloat(lignes[0].getElementsByTagName('td')[2].innerText);
    const domMoy = parseFloat(lignes[0].getElementsByTagName('td')[3].innerText);
    const predDom = (domMax - domMoy < domMoy - domMin) ? "UNDER" : "OVER";
    const ligneDom = Math.round((domMoy + (predDom === "OVER" ? domMin : domMax)) / 2);

    // --- CALCULS ÉQUIPE EXTÉRIEUR ---
    const extMin = parseFloat(lignes[1].getElementsByTagName('td')[1].innerText);
    const extMax = parseFloat(lignes[1].getElementsByTagName('td')[2].innerText);
    const extMoy = parseFloat(lignes[1].getElementsByTagName('td')[3].innerText);
    const predExt = (extMax - extMoy < extMoy - extMin) ? "UNDER" : "OVER";
    const ligneExt = Math.round((extMoy + (predExt === "OVER" ? extMin : extMax)) / 2);

    // --- CALCULS TOTAL MATCH ---
    const tMin = parseFloat(celluleFoot[1].innerText);
    const tMax = parseFloat(celluleFoot[2].innerText);
    const tMoy = parseFloat(celluleFoot[3].innerText);
    const predTot = (tMax - tMoy < tMoy - tMin) ? "UNDER" : "OVER";
    const ligneTot = Math.round((tMoy + (predTot === "OVER" ? tMin : tMax)) / 2);

    // --- CRÉATION DU VISUEL ---
    const divPred = document.createElement('div');
    divPred.id = `pred-box-${zone}`;
    divPred.style = "margin-top:15px; padding:15px; background:rgba(255,215,0,0.05); border:1px solid var(--gold); border-radius:15px;";
    
    const colorDom = (predDom === "OVER") ? "#51cf66" : "#ff922b";
    const colorExt = (predExt === "OVER") ? "#51cf66" : "#ff922b";
    const colorTot = (predTot === "OVER") ? "#51cf66" : "#ff922b";

    divPred.innerHTML = `
        <div style="font-size:0.7em; color:#888; text-transform:uppercase; text-align:center; letter-spacing:1px; margin-bottom:10px;">📊 TENDANCES INDIVIDUELLES</div>
        
        <div style="display:flex; justify-content:space-between; margin-bottom:15px;">
            <div style="text-align:center; width:48%; padding:8px; background:rgba(255,255,255,0.03); border-radius:10px;">
                <div style="font-size:0.65em; color:#888;">🏠 DOMICILE</div>
                <div style="font-weight:bold; color:${colorDom}; font-size:1.1em;">${predDom} ${ligneDom}</div>
            </div>
            <div style="text-align:center; width:48%; padding:8px; background:rgba(255,255,255,0.03); border-radius:10px;">
                <div style="font-size:0.65em; color:#888;">🚌 EXTÉRIEUR</div>
                <div style="font-weight:bold; color:${colorExt}; font-size:1.1em;">${predExt} ${ligneExt}</div>
            </div>
        </div>

        <hr style="border:0; border-top:1px solid rgba(255,215,0,0.2); margin:10px 0;">

        <div style="text-align:center;">
            <div style="font-size:0.75em; color:#888; text-transform:uppercase;">Indicateur Total Match</div>
            <div style="font-size:1.8em; font-weight:900; color:${colorTot}; margin:5px 0;">${predTot} ${ligneTot}</div>
            <div style="font-size:0.8em; color:var(--gold); font-weight:bold;">⭐ FIABILITÉ : TRÈS SÛR</div>
        </div>
    `;
